...
public class DimensionServiceImpl extends ServiceImpl<DimensionDOMapper, DimensionDO>
        ...
        if (CollectionUtils.isEmpty(dimValueMaps.getAlias())) {
            ...
            if (!valeAndMapInfo.containsKey(value)) {
                ...
                List<DimensionValueDO> dimensionValueDOS =
                        insertDimValueAliasInVector(req.getId(), dimensionDO, dimValueMaps);
                ...
                dimValueMapList.forEach(map -> {
                    if (map.getValue().equalsIgnoreCase(value)) {
                        ...
                        if (!CollectionUtils.isEmpty(dimValueMaps.getAlias())) {
                            ...
                            deleteDimValueAlias(req.getId(), value);
                            // 新增向量入库
                            List<DimensionValueDO> dimensionValueDOS = insertDimValueAliasInVector(
                                    req.getId(), dimensionDO, dimValueMaps);
                            sendDimensionValueAliasEventBatch(dimensionValueDOS, EventType.ADD);
                        }
                    }
                });
        ...
        valueMaps.forEach(valueMap -> {
            if (!CollectionUtils.isEmpty(valueMap.getAlias())) {
                ...
                valueMap.getAlias().stream().forEach(alias -> {
                    ...
                    String nature = DictWordType.NATURE_SPILT + dimensionDO.getModelId()
                            + DictWordType.NATURE_SPILT + dimensionDO.getId();
                    name2AliaWord.setNatureWithFrequency(
                            String.format("%s " + DEFAULT_FREQUENCY, nature));
                    dictWordList.add(name2AliaWord);
                });
                KnowledgeBaseService.addDimValueAlias(dimensionDO.getId(), dictWordList);
            }
    ...
    @Override
    public void sendDimensionValueAliasEventBatch(List<DimensionValueDO> dimensionValueAliasList,
            EventType eventType) {
    ...
    private DataEvent getDimValueAliasDataEvent(List<DimensionValueDO> dimensionValueAliasList,
            EventType eventType) {

        List<DataItem> dataItems = dimensionValueAliasList.stream()
                .map(this::getDataItemByDimensionValueAlias).toList();
        return new DataEvent(this, dataItems, eventType);
    }
    ...
    private DataItem getDataItemByDimensionValueAlias(DimensionValueDO dimensionValueDO) {
        ...
        return DataItem.builder().id(dimensionValueDO.getId()).dimId(dimensionValueDO.getDimId())
                ...
                .dimValue(dimensionValueDO.getDimValue()).dimValueAlis(dimensionValueDO.getAlias())
                .build();
    }
     ...
     */
    private List<DimensionValueDO> insertDimValueAliasInVector(Long dimId, DimensionDO dimensionDO,
            DimValueMap dimValueMaps) {

        return dimValueMaps.getAlias().stream().map(tAlias -> {
