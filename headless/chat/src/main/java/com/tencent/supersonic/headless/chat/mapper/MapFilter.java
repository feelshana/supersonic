    ...
    public static void filterByExactMatch(List<SchemaElementMatch> matches) {
        ...
        for (Map.Entry<String, List<SchemaElementMatch>> entry : groupedByDetectWord.entrySet()) {
            ...
            if (!fullMatches.isEmpty()) {
                ...
                // If the word match both of the metaData and the value&the metaData has large
                // similarity
                // delete the value to avoid the LLM Illusion
                if (group.get(0).getElement().getType().equals(SchemaElementType.METRIC) || group
                        .get(0).getElement().getType().equals(SchemaElementType.DIMENSION)) {
                    group = group
                            .stream().filter(schemaElementMatch -> !schemaElementMatch.getElement()
                                    .getType().equals(SchemaElementType.VALUE))
