...
public class OnePassSCSqlGenStrategy extends SqlGenStrategy {
    ...
    public static final String INSTRUCTION = "#角色：你是一位精通SQL语言的数据分析师\n"
            ...
            + "1.Schema中的Dimensions代表维度，Metrics代表指标，Values代表问题分词后，通过向量召回得到的可选维度值列表，供参考，请进行重排序，根据Values中列出的维度，每个维度最多选择一个值作为查询条件\n"
            ...
            + "4.为了防止输出的SQL在使用后返回数据量太大，确保输出的SQL都是限制了最大返回条数的，按照用户问题限制最多返回1000条数据，根据情况在sql添加limit，保证没有语法错误。\n"
