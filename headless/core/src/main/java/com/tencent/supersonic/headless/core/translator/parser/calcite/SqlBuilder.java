...
public class SqlBuilder {
    ...
    public String buildOntologySql(QueryStatement queryStatement) throws Exception {
        ...
        if (!CollectionUtils.isEmpty(ontology.getJoinRelations()) && dataModels.size() > 1) {
            ...
            tableView = render(ontologyQuery, models, scope, schema, semanticSchema,queryStatement.getDimensionRelations());
        } else {
            tableView = render(ontologyQuery, dataModels, scope, schema, semanticSchema,queryStatement.getDimensionRelations());
        }
    ...
    private TableView render(OntologyQuery ontologyQuery, Set<ModelResp> dataModels,
            SqlValidatorScope scope, S2CalciteSchema schema, SemanticSchemaResp semanticSchema,List<String> dimensionRelations)
            throws Exception {
        SqlNode left = null;
    ...
    public static TableView renderOne(Set<MetricSchemaResp> queryMetrics,
            ...
            S2CalciteSchema schema, SemanticSchemaResp semanticSchema,List<String> dimensionRelations) {
        ...
        try {
            ...
            tableView.setWhere(extractDefaultDimValue(semanticSchema, queryDimensions,dimensionRelations));
        } catch (Exception e) {
        ...
        for (Map.Entry<String, String> entry : defaultDimNameMap.entrySet()) {
            ...
            if (filterNameList.contains(defaultDimensionFiledName)) {
                ...
                andConditions.add(notEqualsCall);
                    ...
                    && !hasChildCondtion(defaultDimensionFiledName, filterNameList,dimensionRelations)
            ) {
    ...
    private static boolean hasChildCondtion(String defaultDimensionFiledName, Set<String> filterNameList,List<String> dimRelations) {
        if(CollectionUtils.isEmpty(dimRelations)){
            return false;
        }

//        1.判断是否属于层级分类维度
        String dimRelation=dimRelations.stream().filter(dr->dr.contains(defaultDimensionFiledName)).findFirst().orElse(null);
        if(null==dimRelation){
            return false;
        }
